...
from genertor import generator, evaluator
import pandas as pd
...
# inject_trainable_lora = inject_trainable_lora_extended
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type = float,
        default = 1.0,
        help = "weight for unet model architecture update"
    )
    parser.add_argument(
        "--alpha_text",
        type = float,
        default = 1.0,
        help = "weight for CLIP text encoder update"
    )
...
def main(args):
    ...
    # print(num_params) # number of parameters
    
    # print("model summayr(Unet)",unet.summary())
    ...
    num_params_unet = sum(p.numel() for p in unet.parameters() if p.requires_grad)
    if args.train_text_encoder:
        num_params_clip = sum(p.numel() for p in text_encoder.parameters() if p.requires_grad)
        num_params = num_params_unet + num_params_clip
    else: num_params = num_params_unet
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # make_dot(model_pred,params=dict(unet.named_parameters()), show_attrs=True, show_saved=False).render("rnn_torchviz", format="jpg")
            # exit()
