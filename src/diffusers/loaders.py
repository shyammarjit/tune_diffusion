    ...
    def load_attn_procs(self, pretrained_model_name_or_path_or_dict: Union[str, Dict[str, torch.Tensor]],
        ...
        if is_lora:
            ...
            for key, value_dict in lora_grouped_dict.items():
                if(adapter_type == "krona"):
                    rank = value_dict["to_k_lora.down.weight"].shape[0]
                    hidden_size = value_dict["to_k_lora.up.weight"].shape[0]*rank
                else:
                    rank = value_dict["to_k_lora.down.weight"].shape[0]
                ...
                if isinstance(
                    ...
                    if(adapter_type == "krona"):
                        cross_attention_dim = value_dict["add_k_proj_lora.down.weight"].shape[1]*rank
                    else:
                        cross_attention_dim = value_dict["add_k_proj_lora.down.weight"].shape[1]
                    attn_processor_class = LoRAAttnAddedKVProcessor
                else:
                    if(adapter_type == "krona"):
                        cross_attention_dim = value_dict["to_k_lora.down.weight"].shape[1]*rank
                    else:
...
class LoraLoaderMixin:
    ...
    def load_lora_weights(self, pretrained_model_name_or_path_or_dict: Union[str, Dict[str, torch.Tensor]], 
        ...
        attn_layers = list(state_dict.keys())
    ...
    @classmethod
    def load_lora_into_unet(cls, state_dict, network_alpha, unet, adapter_type=None):
        """
